...
def run():
    ...
    # 使用前一个交易日作为结束日期，避免当日数据不完整
    today = date.today()
    end = today - timedelta(days=1)  # 使用昨天作为结束日期
    
    # 如果昨天是周末，再往前推
    while end.weekday() > 4:  # 0-4是周一到周五，5-6是周末
        end = end - timedelta(days=1)
    
    start = end - timedelta(days=365*5 + 60)  # 5年 + 60天缓冲
    trading_days = generate_trading_days(start, end)
    
    print(f"=== 数据抓取流水线开始执行 ===")
    print(f"执行时间: {pd.Timestamp.now()}")
    print(f"今天日期: {today}")
    print(f"数据结束日期: {end} (避免当日数据不完整)")
    ...
    if os.path.exists(existing_data_file):
        ...
        try:
            ...
            print(f"   读取已有数据失败: {type(e).__name__}: {e}")
            print(f"   将执行全量抓取...")
    ...
    if success_rate < 80:
        print(f"   警告: 成功率过低 ({success_rate:.1f}%)，可能存在网络问题")
